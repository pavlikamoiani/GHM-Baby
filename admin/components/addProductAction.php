...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_product'])) {
	...
	if ($subcategory_id) {
		$stmt->bind_param("ssdssii", $name, $photo, $price, $size, $material, $category_id, $subcategory_id);
	} else {
		$null = null;
		$stmt->bind_param("ssdssii", $name, $photo, $price, $size, $material, $category_id, $null);
	}
	$stmt->execute();
	$stmt->close();
