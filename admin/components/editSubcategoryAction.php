...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_subcategory_id'])) {
	$id = intval($_POST['edit_subcategory_id']);
	$name = $_POST['edit_subcategory_name'] ?? '';
	$photo = '';
	if (!empty($_FILES['edit_subcategory_photo']['name'])) {
		$target_dir = "../../images/subcategories/";
		if (!is_dir($target_dir))
			mkdir($target_dir, 0777, true);
		$tmp_path = $_FILES["edit_subcategory_photo"]["tmp_name"];
		$filename = basename($_FILES["edit_subcategory_photo"]["name"]);
		...
		$stmt = $conn->prepare("UPDATE subcategory SET name=?, photo=? WHERE id=?");
		$stmt->bind_param("ssi", $name, $photo, $id);
	} else {
		$stmt = $conn->prepare("UPDATE subcategory SET name=? WHERE id=?");
