...
// ...функция remove_bg если нужна, можно скопировать из editCategoryAction.php...
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_product_id'])) {
	...
	if (!empty($_FILES['edit_product_photo']['name'])) {
		...
		// }
		$photo = "images/products/" . $filename;
		$stmt = $conn->prepare("UPDATE product SET name=?, category_id=?, subcategory_id=?, photo=? WHERE id=?");
		$stmt->bind_param("siisi", $name, $category_id, $subcategory_id, $photo, $id);
	} else {
		$stmt = $conn->prepare("UPDATE product SET name=?, category_id=?, subcategory_id=? WHERE id=?");
		$stmt->bind_param("siii", $name, $category_id, $subcategory_id, $id);
	}
	$stmt->execute();
	$stmt->close();
	echo "ok";
